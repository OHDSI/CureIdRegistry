...
USE [JHM_OMOP_20220203] 
   --This line will not be relevant for Trayson as his db will be named differently

DROP TABLE

IF EXISTS results.CURE_ID_Cohort;
   WITH covid_lab_pos
   AS (
      ...
      FROM (
         ...
         WHERE condition_concept_id IN (
               ...
               WHERE concept_id IN (
                     ...
                     , 37016114, 46273719, 312940, 36716978, 37395564, 4140438, 46271074, 319049, 314971, 
                     320651
                     )
               )
   ...
   AS (
      SELECT v.person_id, v.visit_occurrence_id, visit_start_date, visit_end_date, p.First_Pos_Date, 
         DATEDIFF(day, p.First_Pos_Date, v.visit_start_date) "Days_From_First_Pos", ABS(DATEDIFF(day, p.
   ...
   INTO Results.CURE_ID_Cohort --Change schema if not using Results
   FROM Vis_Occ v
